...
is_wrapper_type(::Type{SymStruct{T}}) where {T} = true
wraps_type(::Type{SymStruct{T}}) where {T} = T
...
macro symstruct(T, opts = Expr(:block))
    ...
    push!(block.args, quote
        (::$(typeof(has_symwrapper)))(::Type{$T}) where {$(where_args...)} = true
        (::$(typeof(wrapper_type)))(::Type{$T}) where {$(where_args...)} = $SymStruct{$T}
    end)
...
function __field_shape_expr(T::Union{Symbol, Expr}, field::QuoteNode,
    ...
    quote
        function (::$(typeof(field_shape)))(sym::Type{$T}, ::Val{$field}) where {$(where_args...)}
            val = $(esc(val))
